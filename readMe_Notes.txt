GIT repo
git init
git add -A
git commit -m "OK: Reveiews, routing, CSS. Fix: Image in carousel. Do: Venues & Register"
git remote add origin https://github.com/ellingworthl/EWD_Assign1.git
git push -u origin master


readme links
APP BUILD
How To
https://github.com/facebookincubator/create-react-app/blob/master/README.md#getting-started
https://www.kirupa.com/react/creating_single_page_app_react_using_react_router.htm
https://www.youtube.com/watch?v=MhkGQAoc7bc&list=PLoYCgNOIyGABj2GQSlDRjgvXtqfDxKm5b
https://css-tricks.com/css-modules-part-3-react/
http://stackoverflow.com/questions/41515468/custom-stylesheet-in-react-app (install css loader (npm install --save css-loader )
http://pastebin.com/sLvADsEN (css loaders, do npm install --webpack to add then add)
            { test: /\.css$/, loader: "style!css" },
            { test: /\.jade$/, loader: "jade?self" }

Design
https://startbootstrap.com/template-overviews/half-slider/
http://bootsnipp.com/snippets/featured/clean-modal-login-form

TOOLS
http://jsonlint.com/

ADDITIONAL LEARNING
https://egghead.io/